...
```ts
// Portable connection with explicit credentials or a pre-created session.
import { connectCore } from 'browser-pilot/core';

const browser = await connectCore({ provider: 'generic', wsUrl });
try {
  const page = await browser.page();
...
BrowserBase reads `BROWSERBASE_API_KEY` (and optional `BROWSERBASE_PROJECT_ID`, auto-resolved from
the key when exactly one project exists) as an environment fallback for `apiKey`/`projectId`.
Copy [`.env.example`](./.env.example) to `.env` to set these locally. The Node CLI loads `.env`
without overriding existing variables; use `--env-file <path>` to select a file or
