...
(function() {
  ...
  // Also patch enumerateDevices to always report a microphone
...
export class AudioInput {
  ...
  async setup(): Promise<void> {
    ...
    await grantAudioPermissions(this.cdp);

    // Register binding for playback-complete callbacks
    if (!this.bindingRegistered) {
      await this.cdp.send('Runtime.addBinding', { name: INPUT_BINDING });
      this.bindingRegistered = true;
    }

    // Inject script before any page JS runs
  ...
  async play(audioData: ArrayBuffer | Uint8Array, options?: PlayOptions): Promise<void> {
    if (!this.injected) {
      throw new Error('AudioInput not set up. Call setup() first.');
    }
    ...
    if (waitForEnd) {
      // Set up a promise that resolves when the binding fires
