...
(function() {
  ...
  // --- Vector 1: Intercept AudioNode.connect to destination ---
  ...
  // --- Vector 2: Intercept HTMLMediaElement.play ---
...
export class AudioOutput {
  ...
  async setup(): Promise<void> {
    ...
    // Inject capture script before page JS
  ...
  async start(): Promise<void> {
    if (!this.injected) {
      throw new Error('AudioOutput not set up. Call setup() first.');
    }
    this.chunks = [];
