...
import { type ActionTargetMetadata, redactValueForRecording } from '../recording/redaction.ts';
...
export function getHighlightLabel(
  ...
  result: StepResult,
  targetMetadata?: ActionTargetMetadata | null
): string | undefined {
  switch (step.action) {
    case 'fill':
    case 'type':
      return typeof step.value === 'string'
        ? `"${redactValueForRecording(step.value, targetMetadata)}"`
        : undefined;
    case 'select':
